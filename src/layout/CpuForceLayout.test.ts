...
describe("CpuForceLayout", () => {
  it("pulls edge-connected nodes closer together", async () => {
    ...
    const { promise } = new CpuForceLayout().run(nodes, edges, {
      iterations: 80,
    });
  ...
  it("never moves a pinned node", async () => {
    ...
    const edges: LayoutEdge[] = [
      { sourceId: "fixed", targetId: "free", weight: 1 },
    ];
    const { promise } = new CpuForceLayout().run(nodes, edges, {
      iterations: 60,
    });
  ...
  it("groups same-cluster nodes closer than an unrelated node", async () => {
    ...
    const { promise } = new CpuForceLayout().run(nodes, [], {
      iterations: 150,
    });
    const positions = await promise;

    const withinCluster = dist(
      positions.get("rack1-a")!,
      positions.get("rack1-b")!,
    );
    const acrossCluster = dist(
      positions.get("rack1-a")!,
      positions.get("other")!,
    );
