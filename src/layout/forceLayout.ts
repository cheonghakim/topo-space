...
// - Repulsion is pairwise between every non-pinned/other body: O(n^2) per
//   step. That's fine up to roughly a couple thousand nodes; beyond that a
//   GPU compute backend (not yet implemented — see the SDK's scale plan) is
//   the path to real-time layout at 10k+. Pinned nodes still repel others,
//   they just never move themselves.
...
export function stepForceLayout(
  ...
  for (const edge of edges) {
    ...
    if (dist < 1e-4) { dx = 0.1; dz = 0; dist = 0.1 }
    const rest = k / Math.max(edge.weight, 0.1)
    const pull = (dist - rest) * 0.05
    const fx = (dx / dist) * pull
    const fz = (dz / dist) * pull
    if (!a.pinned) { a.vx += fx; a.vz += fz }
    if (!b.pinned) { b.vx -= fx; b.vz -= fz }
  }
