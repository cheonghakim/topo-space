...
export class RaycastManager {
  ...
  get currentPointer(): THREE.Vector2 {
  ...
  private _cast(preferDevice = false): RaycastResult {
    ...
    const spaceHits = this.raycaster.intersectObjects(
      this.spaceRenderer.getHitMeshes(),
      false,
    );
    ...
    if (devHits.length > 0) {
      ...
      const id = this.deviceRenderer.getDeviceIdByInstance(
        type,
        hit.instanceId!,
      );
