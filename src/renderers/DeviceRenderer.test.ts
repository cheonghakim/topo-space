...
afterEach(() => { applyColorMode('default'); disposeGeometryCache() })
...
describe('NMS device rendering', () => {
  it('keeps moved and newly added instances pickable outside the original bounds', () => {
    ...
    const device: RawDevice = { id: 'r1', source: 'test', externalId: 'r1', normalizedType: 'router', status: 'normal' }
    const mapping: DeviceMapping = { id: 'm1', rawDeviceId: 'r1', mappingStatus: 'mapped', position: { x: 0, y: 1, z: 0 } }
    renderer.loadInstanced([device], new Map([['m1', mapping]]), () => mapping)
    const mesh = renderer.getInstancedMeshes()[0]!
    const hitsAt = (x: number) => new THREE.Raycaster(new THREE.Vector3(x, 1, 5), new THREE.Vector3(0, 0, -1)).intersectObject(mesh)
    ...
    renderer.addDevice({ ...device, id: 'r2' }, { ...mapping, id: 'm2', rawDeviceId: 'r2', position: { x: -100, y: 1, z: 0 } })
    expect(hitsAt(-100).length).toBeGreaterThan(0)
    renderer.dispose()
  })
  ...
  it('moves search labels with their devices', () => {
    ...
    const device: RawDevice = { id: 'r1', source: 'test', externalId: 'r1', normalizedType: 'router' }
    const mapping: DeviceMapping = { id: 'm1', rawDeviceId: 'r1', mappingStatus: 'mapped', position: { x: 0, y: 1, z: 0 } }
    renderer.loadInstanced([device], new Map([['m1', mapping]]), () => mapping)
    renderer.setSearchFocus(new Set(['r1']), () => 'router')
    renderer.setPosition('r1', new THREE.Vector3(8, 2, 3))
    renderer.tick()
    const label = renderer.getLabelObstacles().find(object => object.element.className === 'device-search-label')!
  ...
  it('keeps status colors independent of device type and preserves empty search dimming on recolor', () => {
    const renderer = new DeviceRenderer(new THREE.Scene())
    const device: RawDevice = { id: 'r1', source: 'test', externalId: 'r1', normalizedType: 'router', status: 'critical' }
    const mapping: DeviceMapping = { id: 'm1', rawDeviceId: 'r1', mappingStatus: 'mapped', position: { x: 0, y: 1, z: 0 } }
    renderer.loadInstanced([device], new Map([['m1', mapping]]), () => mapping)
    const mesh = renderer.getInstancedMeshes()[0]!
  ...
  it('produces valid, cached, pickable geometries for every built-in type', () => {
    const types: DeviceType[] = ['server', 'switch', 'router', 'firewall', 'database', 'storage', 'vm', 'container', 'load_balancer', 'access_point', 'cloud_service', 'unknown']
    for (const type of types) {
      ...
      expect(geometry.boundingBox!.max.y - geometry.boundingBox!.min.y).toBeGreaterThan(0.1)
    }
  })
})
