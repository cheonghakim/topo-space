...
export class LinkRenderer {
  ...
  private _parallelMidpoint(
    ...
    const px = -dz / len,
      pz = dx / len; // perpendicular unit vector, XZ plane
    return {
      midX: baseMidX + px * step * side,
      midZ: baseMidZ + pz * step * side,
    };
  }
  ...
  setResolution(width: number, height: number) {
    ...
    this.objects.forEach((o) =>
      (o.line.material as LineMaterial).resolution.set(width, height),
    );
    if (this.previewLine)
      (this.previewLine.material as LineMaterial).resolution.set(width, height);
  }

  private _syncAnimated(obj: LinkObj) {
    const needsAnimation =
      obj.link.status === "down" || (obj.line.material as LineMaterial).dashed;
  ...
  private _styleFor(link: NetworkLink) {
    ...
    const linewidth =
      link.type === "physical" || link.type === "security_path" ? 2.5 : 2;
  ...
  updateMidpoint(linkId: string, newX: number, newZ: number) {
    ...
    if (obj.link.midX === undefined && obj.link.midZ === undefined) {
      ...
      if (set) {
        set.delete(linkId);
        if (!set.size) this.byPair.delete(key);
      }
    }
    obj.link.midX = newX;
    obj.link.midZ = newZ;
    ...
    obj.handle.position.set(
      newX,
      this.midY(obj.endpoints.a, obj.endpoints.b),
      newZ,
    );
  }
  ...
  setHighlight(id: string | null, prev: string | null) {
    ...
    if (prev) {
      ...
      if (o)
        (o.line.material as LineMaterial).opacity =
          LINK_STYLE[o.link.type]?.opacity ?? 0.6;
    }
    if (id) {
  ...
  update(delta: number) {
    ...
    this.animated.forEach((id) => {
      ...
      if (obj.link.status === "down") {
        ...
        mat.opacity =
          obj.link.id === this.highlightedId
            ? 1.0
    ...
    if (this.previewLine) {
      (this.previewLine.material as LineMaterial).dashOffset -=
        DASH_FLOW_SPEED * delta;
    }
  }
