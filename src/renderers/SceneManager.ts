...
export class SceneManager {
  ...
  getLabelExclusions(): LabelRect[] {
    ...
    const chrome =
      this._wrapper.parentElement?.querySelectorAll<HTMLElement>(
  ...
  private onWebglContextLost = (event: Event) => {
    ...
    this._onError?.(new Error("WebGL context lost"), {
      phase: "webglcontextlost",
    });
  };

  private onWebglContextRestored = () => {
    this._onError?.(new Error("WebGL context restored; rebuild required"), {
      phase: "webglcontextrestored",
    });
  };
