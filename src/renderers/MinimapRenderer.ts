...
export class MinimapRenderer {
  ...
  render(
    ...
    spaces
      .filter((s) => s.position && !s.archived)
      .forEach((s) => {
        ...
        if (s.type === "rack") {
          ...
          const isRoot =
            s.type === "site" || s.type === "floor" || s.type === "building";
          const sz = s.size ?? { width: 20, depth: 20 };
          const w =
            (sz.width / (this.bounds.maxX - this.bounds.minX)) * (W - 16);
          const h =
            (sz.depth / (this.bounds.maxZ - this.bounds.minZ)) * (H - 16);
