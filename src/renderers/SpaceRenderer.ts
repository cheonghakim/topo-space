...
export class SpaceRenderer {
  ...
  private _buildRack(
    ...
    _space: Space,
    size: Size3DLike,
    colors: { floor: number; edge: number },
  ) {
    const w = size.width,
      d = size.depth;
  ...
  private _buildSite(
    ...
    for (let x = -size.width / 2 + spacing; x < size.width / 2; x += spacing) {
      points.push(
        new THREE.Vector3(x, 0.055, -size.depth / 2),
        new THREE.Vector3(x, 0.055, size.depth / 2),
      );
    }
    for (let z = -size.depth / 2 + spacing; z < size.depth / 2; z += spacing) {
      points.push(
        new THREE.Vector3(-size.width / 2, 0.055, z),
        new THREE.Vector3(size.width / 2, 0.055, z),
      );
    }
    group.add(
  ...
  private _buildZone(
    ...
    const edgeGeo = new THREE.EdgesGeometry(
      new THREE.BoxGeometry(size.width, 0.02, size.depth),
    );
  ...
  setSelected(spaceId: string, on: boolean) {
    ...
    const frame = obj.group.children.find(
      (c) => c instanceof THREE.Mesh && c !== obj.hitMesh,
    ...
    const source = obj.hitMesh.userData.spaceSource as
      Space["source"] | undefined;
  ...
  updateBadge(spaceId: string, text: string) {
    const obj = this.objects.get(spaceId);
    if (obj) {
      obj.badgeEl.textContent = text;
      obj.badgeEl.title = text;
    }
  }
  ...
  applyBadgeLod(cameraDistance = 0) {
    this.objects.forEach((obj) => {
      ...
      const source = obj.hitMesh.userData.spaceSource as
        Space["source"] | undefined;
      obj.badge.visible =
        this.selectedIds.has(obj.group.userData.spaceId) ||
        this.shouldShowBadge(type, source, cameraDistance);
    });
  }
  ...
  updateLod(
    ...
    const occupied = obstacles
      ...
      .map(project)
      .filter((rect): rect is LabelRect => rect !== null);
    ...
    this.objects.forEach((obj, id) => {
      ...
      if (rect)
        candidates.push({
          ...rect,
          id,
          priority: this.selectedIds.has(id)
            ? 100
            : obj.hitMesh.userData.spaceType === "rack"
              ? 40
              : 20,
        });
    });
