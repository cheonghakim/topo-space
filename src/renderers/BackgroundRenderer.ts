...
export class BackgroundRenderer {
  ...
  async addObject(obj: BackgroundObject) {
    ...
    if (obj.kind === 'image') {
      ...
      const w = obj.width ?? 10, d = obj.depth ?? 10
      ...
      root.traverse(child => {
        if (child instanceof THREE.Mesh) {
          ...
          mats.forEach(m => { m.transparent = true; materials.push(m) })
        }
      })
    ...
    root.traverse(child => { child.userData.backgroundId = obj.id })

    const opacity = this.editMode ? EDIT_OPACITY : (obj.opacity ?? DASHBOARD_OPACITY_DEFAULT)
  ...
  setEditMode(active: boolean) {
    ...
    this.objects.forEach(({ materials, obj }) => {
      const opacity = active ? EDIT_OPACITY : (obj.opacity ?? DASHBOARD_OPACITY_DEFAULT)
      materials.forEach(m => { m.opacity = opacity })
    })
  }
