...
function buildSyntheticDataset(count: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    devices.push({
      ...
      externalId: id,
      hostname: `host-${i}`,
      ip: `10.${Math.floor(i / 65536) % 256}.${Math.floor(i / 256) % 256}.${i % 256}`,
...
describe("DeviceRenderer scalability benchmark", () => {
  it("builds a 3,000-device instanced scene within a generous CI time budget", () => {
    ...
    console.log(
      `[bench] DeviceRenderer.loadInstanced(3000 devices) = ${elapsedMs.toFixed(1)}ms (jsdom/CPU)`,
    );
    expect(elapsedMs).toBeLessThan(3000);

    const totalInstances = renderer
      .getInstancedMeshes()
    ...
    expect(renderer.getInstancedMeshes().length).toBeLessThanOrEqual(
      TYPES.length,
    );
