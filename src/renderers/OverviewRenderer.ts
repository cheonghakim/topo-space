...
export class OverviewRenderer {
  ...
  render(cards: OverviewCard[]) {
    ...
    this.cardRects = []
    this.pillRects = []
    this._drillableIds = new Set(cards.filter(c => c.drillable).map(c => c.id))
    ...
    cards.forEach((card, i) => {
      ...
      const h = HEADER_H + (card.drillable
        ? card.children.length * (PILL_H + PILL_GAP) + 12
        : 12)
  ...
  private _drawCard(card: OverviewCard, x: number, y: number, w: number, h: number) {
    const ctx = this.ctx
    ...
    const label = card.critical > 0 ? `${card.critical} CRIT` : card.warning > 0 ? `${card.warning} WARN` : 'OK'
    ...
    if (card.drillable) {
      ...
      card.children.forEach(child => {
        ...
        ctx.fillStyle = pHovered ? 'rgba(30,58,95,0.9)' : 'rgba(255,255,255,0.04)'
        ...
        this.pillRects.push({ x: x + 12, y: py, w: w - 24, h: PILL_H, id: child.id })
        py += PILL_H + PILL_GAP
      })
    }
  }
