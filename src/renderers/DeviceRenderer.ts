...
export class DeviceRenderer {
  ...
  private _updateBadge(deviceId: string, status: DeviceStatus) {
    ...
    if (this.statusBadges.size >= MAX_STATUS_BADGES) {
      ...
      this.badgeSeverity.forEach((s, id) => {
        if (s < weakestSeverity) {
          weakestSeverity = s;
          weakestId = id;
        }
      });
      // Full, and every shown badge is strictly more severe than this one —
      // skip rather than evict; a "+N more" summary is left to the
      // space-level badge rather than growing this set unbounded. On a tie
      // (e.g. two 'critical' devices once the cap is full of criticals),
      // still evict — otherwise every badge past the cap for the *same*
      // top severity would be dropped forever, defeating the cap's purpose
      // during exactly the mass-incident scenario it exists for.
  ...
  loadInstanced(
    ...
    const byType = new Map<
      string,
      { device: RawDevice; mapping: DeviceMapping }[]
    >();
    devices.forEach((dev) => {
    ...
    byType.forEach((items, type) => {
      ...
      items.forEach((item, i) => {
        ...
        if (item.mapping.operatorState?.acknowledged)
          this.ackedIds.add(item.device.id);
      });
    ...
    this.instanceIndex.forEach((_, id) =>
      this._updateBadge(id, this.statusMap.get(id) ?? "unknown"),
    );
  }
    ...
    if (mesh && mesh.count < mesh.instanceMatrix.count) {
      ...
      if (mesh) {
        for (let i = 0; i < mesh.count; i++) {
          ...
          existing.push({
            id,
            pos: new THREE.Vector3().setFromMatrixPosition(m4),
            status: this.statusMap.get(id) ?? ("unknown" as DeviceStatus),
          });
        }
  ...
  updateStatus(deviceId: string, status: DeviceStatus) {
    ...
    if (this.hasFilter && !this.dimmedIds.has(deviceId))
  ...
  setHighlight(deviceId: string, on: boolean) {
    this.highlightedId = on
      ? deviceId
      : this.highlightedId === deviceId
        ? null
        : this.highlightedId;
    if (!this.hoverRing) {
  ...
  setLabelScale(scale: number) {
    ...
    if (this.searchLabels.size) {
      ...
      const texts = new Map(
        [...this.searchLabels.entries()].map(([id, label]) => [
          id,
          label.element.textContent ?? id,
        ]),
      );
    ...
    if (camera && performance.now() >= this.nextTypeLabelUpdate) {
      ...
      for (const { id, pos } of nearby.slice(0, 16)) {
        ...
        if (!label) {
          ...
          el.style.cssText =
            "padding:2px 5px;border-radius:4px;background:#0f172aee;border:1px solid #526780;color:#e2e8f0;font:600 10px monospace;pointer-events:none;white-space:nowrap;";
