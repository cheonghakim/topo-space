...
function makeInterfaces(
  ...
  return portNames.slice(0, count).map((name, i) => ({
    ...
    ip:
      i === 0
        ? `10.${randInt(1, 254)}.${randInt(1, 254)}.${randInt(1, 254)}`
        : undefined,
...
function makeDevice(
  ...
  const ifaceCount =
    type === "switch" ? 8 : type === "router" ? 6 : type === "firewall" ? 4 : 2;
...
export function generateMockData(): MockData {
  ...
  function buildFloorContents(
    parentId: string,
    tag: string,
    rackLabelPrefix: string,
  ) {
    const ZONES = [
    ...
    ZONES.forEach((zone, zi) => {
      ...
      for (let ri = 0; ri < 3; ri++) {
        ...
        spaces.push({
          id: rackId,
          name: `Rack-${rackLabelPrefix}${zi + 1}${ri + 1}`,
          kind: "physical",
          type: "rack",
          parentId: zoneId,
          source: "manual",
        ...
        const hub = rackDevices.find(
          (d) => d.device.normalizedType === "switch",
        );
        if (hub) {
    ...
    if (firstRackIdByZone[0] && firstRackIdByZone[1]) {
      const netSwitch = devices.find(
        (d) =>
          d.normalizedType === "switch" &&
          deviceMappings.find(
            (m) =>
              m.rawDeviceId === d.id &&
              m.primarySpaceId === firstRackIdByZone[0],
          ),
      );
      const srvSwitch = devices.find(
        (d) =>
          d.normalizedType === "switch" &&
          deviceMappings.find(
            (m) =>
              m.rawDeviceId === d.id &&
              m.primarySpaceId === firstRackIdByZone[1],
          ),
      );
  ...
  const seoulRouter = devices.find(
    (d) => d.normalizedType === "router" && d.siteId === "building-seoul",
  );
  const cloudService = devices.find(
    (d) => d.normalizedType === "router" && d.siteId === "site-cloud",
  );
  ...
  const unmappedTypes: DeviceType[] = [
    "server",
    "switch",
    "firewall",
    "server",
