...
export const STATUS_COLOR_HEX: Record<DeviceStatus, string> = reactive({
  ..._DEFAULT_STATUS_COLOR_HEX,
});

export const STATUS_COLOR_THREE: Record<DeviceStatus, THREE.Color> =
  Object.fromEntries(
...
> = {
  ...
  inferred: { color: "#475569", dashed: true, opacity: 0.4 },
};
...
const _customColors = new Map<string, string>();
const _customAbbrs = new Map<string, string>();
const _customLabels = new Map<string, string>();

export function syncCustomTypes(
  types: Map<string, import("@/types").CustomDeviceType>,
) {
  _customColors.clear();
  _customAbbrs.clear();
  _customLabels.clear();
...
export function getTypeColor(type: string): string {
  return (
    _customColors.get(type) ??
    (DEVICE_TYPE_COLOR as Record<string, string>)[type] ??
    "#6b7280"
  );
}
...
export function getTypeAbbr(type: string): string {
  return (
    _customAbbrs.get(type) ??
    (DEVICE_TYPE_ABBR as Record<string, string>)[type] ??
...
export function getTypeLabel(type: string): string {
  return (
    _customLabels.get(type) ??
    (DEVICE_TYPE_LABEL as Record<string, string>)[type] ??
    type
  );
}
