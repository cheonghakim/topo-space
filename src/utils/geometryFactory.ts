...
const DEVICE_SIZE: Record<DeviceType, [number, number, number]> = {
  ...
  switch: [1.0, 0.18, 0.5],
  router: [0.8, 0.22, 0.5],
  firewall: [0.8, 0.24, 0.5],
  database: [0.65, 0.3, 0.55],
  storage: [1.2, 0.3, 0.55],
  vm: [0.6, 0.16, 0.38],
  container: [0.5, 0.16, 0.35],
  load_balancer: [0.8, 0.22, 0.5],
  access_point: [0.55, 0.16, 0.5],
  cloud_service: [0.7, 0.22, 0.5],
  unknown: [0.6, 0.18, 0.4],
};
...
function shade(geo: THREE.BufferGeometry, value = 1): THREE.BufferGeometry {
  geo.setAttribute(
    ...
    new THREE.BufferAttribute(
      new Float32Array(geo.getAttribute("position").count * 3).fill(value),
      3,
    ),
  );
  return geo;
...
function buildAppliance(type: DeviceType): THREE.BufferGeometry {
  ...
  const parts: THREE.BufferGeometry[] = [
    shade(new RoundedBoxGeometry(w, h, d, 2, 0.025), 0.9),
  ];
  const front = d / 2;
  const bays =
    type === "switch" ? 8 : type === "storage" ? 6 : type === "unknown" ? 1 : 4;
  const pitch = (w * 0.76) / bays;
  parts.push(
    shade(
      new THREE.BoxGeometry(w * 0.92, h * 0.72, 0.012).translate(0, 0, front),
      0.22,
    ),
  );
  for (let i = 0; i < bays; i++) {
    ...
    parts.push(
      shade(
        new THREE.BoxGeometry(pitch * 0.82, h * 0.52, 0.018).translate(
          x,
          0,
          front + 0.013,
        ),
    ...
    parts.push(
      shade(
        new THREE.BoxGeometry(pitch * 0.6, h * 0.2, 0.009).translate(
          x,
          0,
          front + 0.026,
        ),
    ...
    parts.push(
      shade(
        new THREE.BoxGeometry(0.016, 0.012, 0.01).translate(
          x,
          h * 0.22,
          front + 0.03,
        ),
  ...
  parts.push(
    shade(
      new RoundedBoxGeometry(unit * 6.4, 0.008, unit * 6.4, 1, 0.003).translate(
        -w * 0.12,
        h / 2,
        0,
      ),
  ...
  LID_SYMBOLS[type].forEach((row, z) => {
    for (let x = 0; x < row.length; x++)
      if (row[x] === "1") {
        parts.push(
          shade(
            new THREE.BoxGeometry(unit * 0.85, 0.008, unit * 0.85).translate(
              (x - 2) * unit - w * 0.12,
              h / 2 + 0.008,
              (z - 2) * unit,
            ),
  ...
  for (let i = 0; i < 5; i++)
    parts.push(
      shade(
        new THREE.BoxGeometry(w * 0.16, 0.003, 0.012).translate(
          w * 0.32,
          h / 2,
          -0.07 + i * 0.035,
        ),
...
export function getDeviceGeometry(type: string): THREE.BufferGeometry {
  ...
  if (custom) {
    ...
    switch (custom.shape) {
      case "cylinder":
        ...
        break;
      case "sphere":
        geo = new THREE.SphereGeometry(custom.w / 2, 10, 8);
        break;
      case "octahedron":
        geo = new THREE.OctahedronGeometry(custom.w / 2);
        break;
      default:
        geo = new THREE.BoxGeometry(custom.w, custom.h, custom.d);
    }
    geo.computeBoundsTree();
    _cache.set(type, geo);
    return geo;
  ...
  const builtIn = Object.prototype.hasOwnProperty.call(DEVICE_SIZE, type)
    ? (type as DeviceType)
    : "unknown";
...
export function getDeviceHeight(type: string): number {
  ...
  return (
    (DEVICE_SIZE as Record<string, [number, number, number]>)[type]?.[1] ?? 0.1
  );
}

export async function preloadCustomModels(
  types: Map<string, CustomDeviceType>,
): Promise<void> {
    ...
    pending.push(
      (async () => {
        try {
          ...
          console.warn(
            `[geometryFactory] Failed to preload model "${type.id}":`,
            e,
          );
        }
      })(),
