...
export class PermissionGuard {
  ...
  setResolver(resolver: PermissionResolver) {
  ...
  can(action: EditorAction, target?: PermissionContext["target"]): boolean {
    ...
    if (action.startsWith("annotation:") && !this.features.annotationEdit)
      return false;
    if (action.startsWith("topology:") && !this.features.topologyEdit)
      return false;
    if (action.startsWith("background:") && !this.features.backgroundEdit)
      return false;
