...
export const useEditorStore = defineStore("editor", () => {
  ...
  const changeLog = ref<
    { id: string; type: string; msg: string; ts: string }[]
  >([]);
  ...
  // The in-flight run plus whether cancelAutoLayout() was asked to stop it.
  // Identity-compared in the run's own .then() so a run that's been
  // superseded by a newer autoLayout() call (rather than explicitly
  // cancelled) can tell and skip applying its now-stale results entirely.
  let _current: { run: LayoutRun; cancelledByUser: boolean } | null = null;
  ...
  function can(
    action: EditorAction,
    target?: PermissionContext["target"],
  ): boolean {
  ...
  function emitChange(
    ...
    source: EditorEventPayload["source"] = "user",
  ) {
  ...
  const mappedDeviceIds = computed(() => {
    ...
    mappings.value.forEach((m) => {
      if (m.mappingStatus === "mapped" || m.mappingStatus === "auto_mapped")
        ids.add(m.rawDeviceId);
    });
    return ids;
  });
  ...
  function childSpaces(parentId: string): Space[] {
    return [...spaces.value.values()].filter(
      (s) => s.parentId === parentId && !s.archived,
    );
  }
  ...
  function scopedLinks(rootId: string | null): NetworkLink[] {
    ...
    const ids = scopedDeviceIds(rootId);
    return [...links.value.values()].filter(
      (l) => ids.has(l.sourceDeviceId) && ids.has(l.targetDeviceId),
    );
  }
  ...
  function scopedBackgroundObjects(rootId: string | null): BackgroundObject[] {
    ...
    const ids = descendantSpaceIds(rootId);
    return [...backgroundObjects.value.values()].filter((b) =>
      ids.has(b.spaceId),
    );
  }
  ...
  function resolveLeafScope(spaceId: string): string | null {
    ...
    let id = cur.id;
    for (;;) {
      const containerChild = childSpaces(id).find((c) =>
  ...
  function replaceData(data: EditorData) {
    ...
    interfaces.value = new Map((data.interfaces ?? []).map((i) => [i.id, i]));
    unmappedDevices.value = [...(data.unmappedDevices ?? [])];
    virtualNodes.value = new Map(
      (data.virtualNodes ?? []).map((n) => [n.id, n]),
    );
    backgroundObjects.value = new Map(
      (data.backgroundObjects ?? []).map((b) => [b.id, b]),
    );
  }
  ...
  function removeDevices(ids: string[]) {
    ...
    ids.forEach((id) => {
      ...
      const mapEntry = [...mappings.value.entries()].find(
        ([, m]) => m.rawDeviceId === id,
      );
    ...
    ids.forEach((id) =>
      emitChange("device:unmap", { id, type: "device" }, "api"),
    );
  }
  ...
  function addSpace(space: Space) {
    if (!requirePermission("space:create", { id: space.id, spaceId: space.id }))
      return;
  ...
  function mapDevice(
    ...
    const existing = [...mappings.value.values()].find(
      (m) => m.rawDeviceId === deviceId,
    );
  ...
  function unmapDevice(deviceId: string) {
    ...
    const entry = [...mappings.value.entries()].find(
      ([, m]) => m.rawDeviceId === deviceId,
    );
    if (!entry) return;
    const [key, m] = entry;
  ...
  function updateAnnotation(
    deviceId: string,
    patch: { displayName?: string; tags?: string[]; memo?: string },
  ) {
    if (!requirePermission("annotation:update", { id: deviceId })) return;
    const m = [...mappings.value.values()].find(
      (m) => m.rawDeviceId === deviceId,
    );
  ...
  function acknowledgeDevice(deviceId: string, by = "operator") {
    ...
    logChange(
      "device.ack",
      `Acknowledged: ${devices.value.get(deviceId)?.hostname ?? deviceId}`,
    );
    emitChange("annotation:update", { id: deviceId, type: "device" });
  }
  ...
  function addBackgroundObject(obj: BackgroundObject) {
    if (
      !requirePermission("background:create", {
        id: obj.id,
        spaceId: obj.spaceId,
      })
    )
  ...
  function updateBackgroundObject(
    id: string,
    patch: Partial<BackgroundObject>,
  ) {
  ...
  function importTopology(
    ...
    if (!requirePermission("import"))
      return { devices: 0, spaces: 0, links: 0 };
    ...
    type Group = Map<
      string,
      { building?: string; zones: Map<string, Map<string, typeof rows>> }
    >;
    ...
    floors.forEach(
      ([floorName, { building: buildingName, zones: zoneMap }], fi) => {
        ...
        const zoneLayouts = zones.map(([zoneName, rackMap]) => {
          ...
          const zoneW =
            racks.reduce((a, r) => a + r.w, 0) +
            Math.max(0, racks.length - 1) * RACK_GAP +
            ZONE_PAD;
        ...
        const floorW =
          zoneLayouts.reduce((a, z) => a + z.zoneW, 0) +
          Math.max(0, zoneLayouts.length - 1) * ZONE_GAP +
          SITE_PAD;
        const floorD =
          Math.max(...zoneLayouts.map((z) => z.zoneD), 1) + SITE_PAD;
          ...
          racks.forEach(({ rackName, devs, w, d, rows: rows2 }, ri) => {
            ...
            devs.forEach((r, slot) => {
              ...
              const dev: RawDevice = {
                ...
                ip: r.ip,
                normalizedType:
                  (r.type as RawDevice["normalizedType"]) ?? "unknown",
                vendor: r.vendor,
  ...
  // persistence path.
  //
  // Without an explicit `deviceIds`, only devices that already have a
  // DeviceMapping are considered — a device that was never mapped (or was
  // explicitly unmapped) has no space/rack context to place it in, so
  // sweeping it in here would silently promote it to "mapped" with no
  // space at all. A host that wants those included must pass their ids
  // explicitly, e.g. scoped to the currently viewed floor.
  function autoLayout(
    options: AutoLayoutOptions = {},
  ): Promise<{ cancelled: boolean }> {
    if (!requirePermission("layout:update"))
      return Promise.resolve({ cancelled: true });

    const mappingByDevice = new Map<string, DeviceMapping>();
    mappings.value.forEach((m) => mappingByDevice.set(m.rawDeviceId, m));

    const targetIds = (options.deviceIds ?? [...mappingByDevice.keys()]).filter(
      (id) => devices.value.has(id),
    );
    ...
    const nodes: LayoutNode[] = targetIds.map((id) => {
      ...
      return {
        ...
        z: start.z,
        pinned,
        clusterId: mapping?.primarySpaceId,
      };
    });
    ...
    links.value.forEach((l) => {
      ...
      edges.push({
        sourceId: l.sourceDeviceId,
        targetId: l.targetDeviceId,
        weight: _layoutEdgeWeight(l),
      });
    });

    // A newer call supersedes whatever's in flight — cancelling it here
    // means its promise resolves soon with a stale/partial snapshot, so the
    // `.then()` below identity-checks `_current` and bails out entirely
    // rather than letting that stale run clobber this one's results.
    _current?.run.cancel();

    const iterations = options.iterations ?? 200;
    autoLayoutProgress.value = { iteration: 0, iterations };
    const run = new CpuForceLayout().run(nodes, edges, { iterations }, (p) => {
      autoLayoutProgress.value = {
        iteration: p.iteration,
        iterations: p.iterations,
      };
      options.onProgress?.(p.iterations ? p.iteration / p.iterations : 1);
    });
    const entry = { run, cancelledByUser: false };
    _current = entry;

    return run.promise.then((positions) => {
      const superseded = _current !== entry;
      ...
      nodes.forEach((n) => {
        ...
        const spaceId = mapping?.primarySpaceId ?? "";
        // Checked ahead of the call so a denial doesn't get counted as a
        // move — mapDevice() still runs its own identical check and notifies
        // the host as usual if it's denied.
        const allowed = can("device:map", { id: n.id, spaceId });
        mapDevice(n.id, spaceId, mapping?.slotIndex ?? 0, {
          x: pos.x,
          y: mapping?.position?.y ?? 0,
          z: pos.z,
        });
        if (allowed) movedIds.push(n.id);
      });
      if (movedIds.length) {
        logChange(
          "layout.update",
          `Auto layout placed ${movedIds.length} device(s)`,
        );
        lastAutoLayoutDeviceIds.value = movedIds;
      }
      return { cancelled: entry.cancelledByUser };
    });
  }

  function cancelAutoLayout() {
    if (_current) _current.cancelledByUser = true;
    _current?.run.cancel();
  }
  ...
  function exportSnapshot(): EditorSnapshot {
    return {
      ...
      manualLinks: [...links.value.values()].filter(
        (l) => l.source === "manual",
      ),
  ...
  function importSnapshot(snap: EditorSnapshot) {
    ...
    (safeSnap.backgroundObjects ?? []).forEach((b) =>
      backgroundObjects.value.set(b.id, b),
    );
    unmappedDevices.value = unmappedDevices.value.filter(
      (d) =>
        !safeSnap.deviceMappings.find(
          (m) => m.rawDeviceId === d.id && m.mappingStatus === "mapped",
        ),
    );
  }
  ...
  function readonlyDevice(dev: RawDevice): Readonly<RawDevice> {
    return new Proxy(dev, {
      set() {
        throw new TypeError(
          "RawDevice is read-only. Use explicit editor commands for changes.",
        );
      },
      deleteProperty() {
        throw new TypeError(
