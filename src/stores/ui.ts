...
export const useUIStore = defineStore("ui", () => {
  ...
  const visibleLinkTypes = ref<Set<EdgeType>>(
    new Set([
      "physical",
      "logical",
      "service_dependency",
  ...
  // Dims every 'normal' device regardless of the search/status/type filters —
  // a fast way to see nothing but the current alarms in a dense scene.
  ...
  const colorblindMode = ref(
    localStorage.getItem("topospace.colorblindMode") === "1",
  );
  function setColorblindMode(on: boolean) {
    colorblindMode.value = on;
    localStorage.setItem("topospace.colorblindMode", on ? "1" : "0");
  }
  ...
  // Critical/warning devices that are currently off-screen or hidden behind
  // other geometry — surfaced as clickable edge arrows so an operator never
  // has to stumble on a hidden alarm by rotating the camera at random.
  const offscreenAlerts = ref<
    {
      id: string;
  ...
  function closeLeftDock() {
    ...
    showCustomTypes.value = false;
    showBackgroundPanel.value = false;
    showRackServerList.value = false;
    showSpaceTree.value = false;
    showUnmapped.value = false;
    selectedRackForList.value = null;
  }
