    ...
    timers.push(
      setInterval(() => {
        ...
        if (newStatus === "critical") {
          ...
          newStatus === "normal" &&
          (oldStatus === "critical" || oldStatus === "warning")
        ) {
      ...
      setInterval(() => {
        ...
        sample.forEach((id) => {
          ...
          editor.updateDeviceStatus(id, dev.status ?? "normal", {
            cpu: Math.min(
              100,
              Math.max(0, (dev.metrics.cpu ?? 50) + (Math.random() - 0.5) * 15),
            ),
            memory: Math.min(
              100,
              Math.max(
                0,
                (dev.metrics.memory ?? 50) + (Math.random() - 0.5) * 8,
              ),
            ),
            networkIn: Math.max(
              0,
              (dev.metrics.networkIn ?? 100) + (Math.random() - 0.5) * 200,
            ),
            networkOut: Math.max(
              0,
              (dev.metrics.networkOut ?? 50) + (Math.random() - 0.5) * 100,
            ),
          });
        });
      }, 1500),
