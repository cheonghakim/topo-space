...
const TEXT: Record<Lang, Record<'welcome' | 'campus' | 'canvas' | 'mode' | 'connect', { title: string; description: string }>> = {
  en: {
    welcome: {
...
export function startProductTour(lang: Lang = detectLang()) {
  ...
  const hasHierarchy =
    ...
    editor.rootSpaces.some(r => editor.childSpaces(r.id).some(c => CONTAINER_TYPES.has(c.type)))
  ...
  if (hasHierarchy && document.querySelector('.vs-btn')) {
    steps.push({ element: '.vs-btn', popover: { ...t.campus, side: 'bottom', align: 'start' } })
  }
  if (document.querySelector('.canvas-wrap')) {
    steps.push({ element: '.canvas-wrap', popover: { ...t.canvas, side: 'bottom', align: 'center' } })
  }
  if (document.querySelector('.mode-switch')) {
    steps.push({ element: '.mode-switch', popover: { ...t.mode, side: 'bottom', align: 'end' } })
  }
  if (document.querySelector('[title="Connect devices (L)"]')) {
    steps.push({ element: '[title="Connect devices (L)"]', popover: { ...t.connect, side: 'bottom', align: 'end' } })
  }
