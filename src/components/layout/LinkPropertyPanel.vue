  ...
  <aside v-if="link" class="panel">
    <div class="panel-head">
      <div class="link-title">
        <span class="link-icon" :style="{ color: LINK_STYLE[link.type]?.color }"
          >╌</span
        >
        <div>
    ...
    <div class="panel-body">
      <section class="section">
        ...
        <div class="info-grid">
          ...
          <span class="v">{{ LINK_LABEL[link.type] }}</span>
          <span class="k">Status</span>
          <span class="v" :class="`s-${link.status}`">{{
            link.status ?? "unknown"
          }}</span>
          <span class="k">Source</span> <span class="v">{{ link.source }}</span>
          <span class="k">Confidence</span>
          <span class="v">{{ link.confidence ?? "—" }}</span>
          <span class="k">Bandwidth</span>
          <span class="v">{{
            link.bandwidth ? link.bandwidth + " Mbps" : "—"
          }}</span>
          <span class="k">Label</span>
          <span class="v">{{ link.label ?? "—" }}</span>
        </div>
      </section>

      <section
        v-if="ui.mode === 'edit' && link.source === 'manual'"
        class="section"
      >
...
const link = computed(() =>
  ui.selectedLinkId ? (editor.links.get(ui.selectedLinkId) ?? null) : null,
);
const srcDev = computed(() =>
  link.value ? editor.devices.get(link.value.sourceDeviceId) : null,
);
const tgtDev = computed(() =>
  link.value ? editor.devices.get(link.value.targetDeviceId) : null,
);
...
watch(
  link,
  (l) => {
    editStatus.value = l?.status ?? "up";
    editLabel.value = l?.label ?? "";
  },
  { immediate: true },
);
...
function save() {
  ...
  editor.updateLink(link.value.id, {
