<template>
  <div v-if="ui.showTimeline" class="tl-panel">
    <div class="tl-head">
      <button :class="['tl-btn', recording ? 'rec' : '']" @click="toggleRecord">
        {{ recording ? "Stop Recording" : "Record" }}
      </button>
      <span v-if="frameCount > 0" class="tl-info">{{ frameCount }} frames</span>
      <div class="tl-spacer" />
      <button class="tl-btn" title="Export timeline" @click="exportTimeline">
        Export Timeline
      </button>
      <button class="tl-btn" title="Export layout" @click="exportLayout">
        Export Layout
      </button>
      <button class="tl-btn" title="Import layout" @click="triggerImport">
        Import Layout
      </button>
      <input
        ref="fileInput"
    ...
    <div v-if="frameCount > 0" class="tl-slider-wrap">
  ...
  if (recording.value) {
    ...
    props.timeline.startRecording(() => {
      ...
      editor.devices.forEach((d) => {
        state[d.id] = { status: d.status, metrics: { ...d.metrics } };
      });
      return state;
    });
