<template>
  <aside v-if="device" class="panel">
    <div class="panel-head">
      <div class="dev-title">
        <span
          class="type-tag"
          :style="{
            color: typeColor(device.normalizedType),
            borderColor: typeColor(device.normalizedType),
          }"
        >
        ...
        <div>
          <div class="dev-name">
            {{ mapping?.displayName ?? device.hostname }}
          </div>
        ...
        <span
          class="status-badge"
          :style="{
            color: STATUS_COLOR_HEX[device.status ?? 'unknown'],
            background: '#17263b',
          }"
          >{{ STATUS_ICON[device.status ?? "unknown"] }}
          {{ STATUS_LABEL[device.status ?? "unknown"] }}</span
        >
        <span
          v-if="mapping?.operatorState?.acknowledged"
          class="ack-badge"
          title="Acknowledged — original status is preserved above"
          >✓ Ack</span
        >
        <button class="close-btn" title="Close" @click="ui.select(null)">
          ✕
        </button>
      </div>
    </div>
    ...
    <div class="panel-body">
      <section class="section">
        ...
        <div class="info-grid">
          <span class="info-k">Vendor</span>
          <span class="info-v">{{ device.vendor ?? "—" }}</span>
          <span class="info-k">Model</span>
          <span class="info-v">{{ device.model ?? "—" }}</span>
          <span class="info-k">OS</span>
          <span class="info-v">{{ device.os ?? "—" }}</span>
          <span class="info-k">Source</span>
          <span class="info-v">{{ device.source }}</span>
          <span class="info-k">Sync</span>
          <span class="info-v">{{ device.syncState }}</span>
        </div>
      </section>

      <section v-if="device.metrics" class="section">
        ...
        <div v-for="m in metrics" :key="m.key" class="metric-row">
          ...
          <span class="m-val" :class="{ hot: m.pct > 85 }">{{
            m.display
          }}</span>
        </div>
      </section>
      ...
      <section v-if="ui.mode === 'edit'" class="section actions">
        ...
        <div class="action-btns">
          ...
          <button
            class="act-btn ack"
            @click="
              mapping?.operatorState?.acknowledged
                ? unacknowledge()
                : acknowledge()
            "
          >
...
const device = computed(() =>
  ui.selectedDeviceId
    ? (editor.devices.get(ui.selectedDeviceId) ?? null)
    : null,
);
const mapping = computed(() =>
  device.value ? (editor.getMappingByDeviceId(device.value.id) ?? null) : null,
);
const ifaces = computed(() =>
  device.value ? (editor.interfacesByDevice.get(device.value.id) ?? []) : [],
);
const upCount = computed(
  () => ifaces.value.filter((i) => i.status === "up").length,
);
const dnCount = computed(
  () => ifaces.value.filter((i) => i.status === "down").length,
);
...
const metrics = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      label: "Memory",
      pct: m.memory ?? 0,
      display: `${(m.memory ?? 0).toFixed(1)}%`,
      color: bar(m.memory ?? 0),
    },
    {
      key: "disk",
      label: "Disk",
      pct: m.disk ?? 0,
      display: `${(m.disk ?? 0).toFixed(1)}%`,
      color: bar(m.disk ?? 0),
    },
    {
      key: "netin",
      label: "Net In",
      pct: Math.min((m.networkIn ?? 0) / 10, 100),
      display: `${(m.networkIn ?? 0).toFixed(0)} M`,
      color: "#3b82f6",
    },
    {
      key: "temp",
      label: "Temp",
      pct: Math.min((((m.temperature ?? 40) - 20) / 60) * 100, 100),
      display: `${(m.temperature ?? 0).toFixed(0)}°C`,
      color: bar((((m.temperature ?? 40) - 20) / 60) * 100),
    },
  ];
});

function bar(pct: number) {
  return pct >= 90 ? "#ef4444" : pct >= 70 ? "#eab308" : "#22c55e";
}
function fmt(n?: number) {
  if (!n) return "0";
  return n >= 1000 ? `${(n / 1000).toFixed(1)}G` : `${n.toFixed(0)}M`;
}

function isolate() {
  if (device.value) editor.updateDeviceStatus(device.value.id, "offline");
  actionLog.value = `[${ts()}] Isolated (simulated)`;
}
function recover() {
  if (device.value) editor.updateDeviceStatus(device.value.id, "normal");
  actionLog.value = `[${ts()}] Recovered (simulated)`;
}
...
function saveAssignment() {
  ...
  actionLog.value = assignee.value.trim()
    ? `[${ts()}] Assigned to ${assignee.value.trim()}`
    : `[${ts()}] Unassigned`;
}
...
.iface-row {
  display: grid;
  grid-template-columns: 8px 55px 40px 90px 35px 1fr auto;
  align-items: center;
  gap: 4px;
  padding: 3px 4px;
