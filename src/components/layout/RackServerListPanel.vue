<template>
  <aside v-if="rack" class="panel">
    ...
    <div class="hint">
      Click a device to open details · hover to highlight in 3D.
    </div>
    ...
    <div class="server-list">
      ...
      >
        <span
          class="dot"
          :style="{ background: STATUS_COLOR_HEX[dev.status ?? 'unknown'] }"
        />
        <span
          class="type-tag"
          :style="{
        ...
        <div v-if="dev.metrics" class="srv-metrics">
          ...
          >
          <span :class="{ hot: (dev.metrics.memory ?? 0) > 85 }"
            >M{{ (dev.metrics.memory ?? 0).toFixed(0) }}%</span
          >
        </div>
      </div>
    </div>
...
const rack = computed(() =>
  ui.selectedRackForList
    ? (editor.spaces.get(ui.selectedRackForList) ?? null)
    : null,
);
...
const critCount = computed(
  () => rackDevices.value.filter((d) => d.status === "critical").length,
);
const warnCount = computed(
  () => rackDevices.value.filter((d) => d.status === "warning").length,
);
const normCount = computed(
  () => rackDevices.value.filter((d) => d.status === "normal").length,
);
