<template>
  <div class="cl-panel">
    <div class="cl-head">
      ...
      <button class="cl-clear" @click="editor.changeLog.splice(0)">
        Clear
      </button>
    ...
    <div class="cl-list">
      <div v-for="entry in filteredLog" :key="entry.id" class="cl-row">
        <span class="cl-type" :class="entryClass(entry.type)">{{
          entry.type
        }}</span>
...
const filteredLog = computed(() => {
  ...
  return editor.changeLog.filter(
    (e) => entryClass(e.type) === typeFilter.value,
  );
});
