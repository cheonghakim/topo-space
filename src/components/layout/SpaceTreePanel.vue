...
const customGroups = computed(() => [...editor.spaces.values()].filter(s =>
  ['custom_group', 'security_zone', 'service', 'external', 'cloud'].includes(s.type) && !s.archived))

function getChildren(parentId: string, type: SpaceType) {
  return [...editor.spaces.values()].filter(s => s.parentId === parentId && s.type === type && !s.archived)
}
...
function rackHasIssue(rackId: string) {
  return (editor.devicesBySpace.get(rackId) ?? []).some(d => d.status === 'critical' || d.status === 'warning')
}
...
function confirmAdd() {
  ...
  const parentSpace = addChildParentId.value ? editor.spaces.get(addChildParentId.value) : null
  const basePos = parentSpace?.position ?? { x: 0, y: 0, z: 0 }

  editor.addSpace({
    id, name: newName.value.trim(),
    kind: ['site','zone','rack'].includes(newType.value) ? 'physical' : 'logical',
    type: newType.value,
    parentId: addChildParentId.value ?? undefined,
    source: 'manual',
    position: { x: basePos.x + Math.random() * 10 - 5, y: 0, z: basePos.z + Math.random() * 10 - 5 },
    size: newType.value === 'rack' ? { width: 1, height: 3, depth: 0.6 } :
          newType.value === 'zone' ? { width: 20, height: 0.1, depth: 18 } :
...
.tree-row.site    { padding-left: 6px; }
.tree-row.zone    { padding-left: 18px; background: rgba(255,255,255,.02); }
...
.row-btn:hover     { color: #94a3b8; }
.row-btn.del:hover { color: #ef4444; }
.add-child-btn {
  padding: 4px 8px 4px 32px; color: #334155; font-size: 10px;
  cursor: pointer; border-bottom: 1px solid rgba(255,255,255,.02);
}
.add-child-btn:hover { color: #3b82f6; background: rgba(59,130,246,.06); }
.add-child-btn.root  { padding-left: 8px; border-top: 1px solid #1a2a4a; }

.add-modal {
  position: absolute; bottom: 0; left: 0; right: 0;
  background: rgba(9,13,24,.98); border-top: 1px solid #2a4a8a;
  padding: 12px; z-index: 200;
}
.add-title { color: #94a3b8; font-size: 11px; margin-bottom: 8px; }
.add-sel, .add-input {
  width: 100%; background: #0f172a; border: 1px solid #1e3a5a; color: #e2e8f0;
  padding: 5px 8px; border-radius: 5px; font-size: 11px; outline: none; margin-bottom: 6px;
}
.add-btns { display: flex; gap: 6px; }
.add-ok     { flex: 1; background: #1e3a5f; border: 1px solid #2a4a8a; color: #93c5fd; padding: 5px; border-radius: 5px; cursor: pointer; font-size: 11px; }
.add-cancel { flex: 1; background: transparent; border: 1px solid #334155; color: #64748b; padding: 5px; border-radius: 5px; cursor: pointer; font-size: 11px; }
