    ...
    <div v-if="ui.showImport" class="imp-overlay" @click.self="close">
      <div class="imp-modal">
        ...
        <div class="imp-body">
          ...
          <section class="imp-section">
            ...
            >
              ...
              <span v-if="fileName" class="file-name"
                >{{ fileName }} · {{ result?.rows.length ?? 0 }} rows</span
              >
            </div>
            <div class="schema">
              Expected columns:
              <code>hostname</code>, <code>ip</code>, <code>type</code>,
              <code>vendor</code>, <code>building</code>, <code>floor</code> (or
              legacy <code>site</code>), <code>zone</code>, <code>rack</code>,
              <code>status</code>, <code>uplink</code>.
              <span class="req">hostname</span> and
              <span class="req">type</span> are required;
              <code>building</code> is optional and only wraps floors when
              supplied.
            </div>
          </section>

          <!-- Step 2: preview -->
          <section v-if="result" class="imp-section">
            <div class="step-title">2. Preview</div>

            <div v-if="result.errors.length" class="err-block">
              <div
                v-for="(e, i) in result.errors.slice(0, 5)"
                :key="i"
                class="err-line"
              >
                {{ e }}
              </div>
              <div v-if="result.errors.length > 5" class="err-more">
                … and {{ result.errors.length - 5 }} more
              </div>
            </div>
            <div v-if="result.warnings.length" class="warn-block">
              <div v-for="(w, i) in result.warnings" :key="i" class="warn-line">
                {{ w }}
              </div>
            </div>

            <div v-if="result.rows.length" class="tbl-wrap">
              <table class="tbl">
                <thead>
                  <tr>
                    <th>hostname</th>
          ...
          <!-- Step 3: options -->
          <section v-if="result && result.rows.length" class="imp-section">
            <div class="step-title">3. Import options</div>
            <label class="opt">
              <input v-model="replaceAll" type="checkbox" />
              <span
                >Replace current scene (clear existing devices, spaces,
                links)</span
              >
            </label>
            ...
            <button
              class="run-btn"
              :disabled="ui.mode !== 'edit' || importing"
              @click="run"
            >
...
async function loadFile(file: File) {
  ...
  try {
    ...
    result.value = {
      rows: [],
      errors: [`Failed to parse: ${(err as Error).message}`],
      warnings: [],
    };
  }
}
...
async function run() {
  ...
  if (replaceAll.value) {
    if (
      !confirm(
        "Replace the current scene? This will remove existing devices, spaces, and links.",
      )
    ) {
...
.imp-body::-webkit-scrollbar {
  width: 3px;
}
.imp-body::-webkit-scrollbar-track {
  background: transparent;
}
.imp-body::-webkit-scrollbar-thumb {
  background: #1a2a4a;
  border-radius: 2px;
}
.imp-body::-webkit-scrollbar-thumb:hover {
  background: #243656;
}
.imp-section {
  margin-top: 14px;
...
.imp-fade-enter-active,
.imp-fade-leave-active {
