...
const cards = computed<OverviewCard[]>(() =>
  editor.rootSpaces.map(root => {
    const containerChildren = editor.childSpaces(root.id).filter(c => CONTAINER_TYPES.has(c.type))
    ...
    return {
      ...
      children: containerChildren.map(c => {
        ...
        return { id: c.id, name: c.name, critical: cc.critical, warning: cc.warning }
      }),
    }
  }),
)
...
.ov-canvas { display: block; width: 100%; height: 100%; }
.ov-empty {
  position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);
  color: #475569; font-size: 13px; font-family: monospace;
  pointer-events: none;
}
</style>
