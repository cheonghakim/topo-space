...
export async function parseXlsx(buffer: ArrayBuffer): Promise<ParseResult> {
  ...
  if (!sheet) {
    result.errors.push("No worksheet found in the file.");
    return result;
  }
  ...
  while (
    ...
    sheet.getRow(headerRowIndex).actualCellCount === 0
  ) {
    headerRowIndex++;
  }
  ...
  for (let r = headerRowIndex + 1; r <= sheet.rowCount; r++) {
    ...
    if (error) {
      result.errors.push(`Row ${r}: ${error}`);
      continue;
    }
