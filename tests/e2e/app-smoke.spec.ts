...
test('renders the editor shell and keeps edit-only controls out of view mode', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'View', exact: true })).toHaveClass(/active/)
...
test('offers camera navigation and an accessible controls guide', async ({ page }) => {
  const errors: string[] = []
  page.on('pageerror', error => errors.push(error.message))
  await page.goto('/')
  const navigation = page.getByRole('navigation', { name: 'Camera controls' })
  await expect(navigation.getByRole('button', { name: 'Focus selected' })).toBeDisabled()
  ...
  await navigation.getByRole('button', { name: 'Top view', exact: true }).click()
  await navigation.getByRole('button', { name: 'Zoom in', exact: true }).click()
  await navigation.getByRole('button', { name: 'Zoom out', exact: true }).click()
  await navigation.getByRole('button', { name: '3D view', exact: true }).click()
...
test('explains status icons and updates the visible palette and alerts together', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  await page.goto('/')
  const legend = page.getByRole('region', { name: 'Status legend' })
  await expect(legend.locator('[data-status="critical"]')).toContainText('Critical')
  ...
  const before = await criticalIcon.evaluate(el => getComputedStyle(el).color)
  const toggle = legend.getByRole('button', { name: 'Standard palette', exact: true })
  await toggle.click()
  await expect(legend.getByRole('button', { name: 'Colorblind palette on' })).toHaveAttribute('aria-pressed', 'true')
  await expect.poll(() => criticalIcon.evaluate(el => getComputedStyle(el).color)).not.toBe(before)
...
test('fits the SDK into its host container and can be destroyed', async ({ page }) => {
  await page.goto('/')
  await page.evaluate(async () => {
    ...
    const sdk = createNmsEditor({ container: host, mockData: false, data: { devices: [], spaces: [] } })
    ;(window as unknown as { disposeNms: () => void }).disposeNms = () => sdk.destroy()
  })
  ...
  await expect(host.getByRole('button', { name: /Status legend/ })).toHaveAttribute('aria-expanded', 'false')
  await page.evaluate(() => (window as unknown as { disposeNms: () => void }).disposeNms())
  await expect(host.locator('canvas')).toHaveCount(0)
})

test('filters alerts by severity and keeps selection available', async ({ page }) => {
  await page.goto('/')
  const panel = page.getByRole('complementary', { name: 'Alerts', exact: true })
  await panel.getByRole('group', { name: 'Alert severity' }).getByRole('button', { name: /Critical/ }).click()
  await expect(panel.locator('.ap-device.critical').first()).toBeVisible()
  await expect(panel.locator('.ap-device.warning, .ap-device.offline')).toHaveCount(0)
  const first = panel.locator('.ap-device.critical').first()
  await first.click()
  await expect(first).toHaveAttribute('aria-pressed', 'true')
  await expect(page.getByRole('button', { name: 'Focus selected' })).toBeEnabled()
})

test('keeps visible rack names inside the viewport and prevents name collisions', async ({ page }) => {
  await page.goto('/')
  for (const name of ['Top view', '3D view']) {
    await page.getByRole('button', { name, exact: true }).click()
    await page.waitForTimeout(800)
    const result = await page.evaluate(() => {
      const canvas = document.querySelector('.scene-wrap')!.getBoundingClientRect()
      ...
      return { count: labels.length, overlaps, clipped: labels.some(r => r.left < canvas.left || r.right > canvas.right || r.top < canvas.top || r.bottom > canvas.bottom) }
    })
